...
- [ ] add properly in ODRL Evaluater by creating a specific ODRL Evaluator
...
Request policy evaluation for a compact rule with two actions would result in two action reports, where only one of them would be satisfied.
...
const atomizedRules = await atomizer.atomizePolicies(compactPolicyQuads);
// evaluate TODO: make easier
